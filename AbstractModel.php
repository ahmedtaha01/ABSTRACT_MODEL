...
class AbstractModel{
    ...
    public function where($array){
        ...
        foreach($array as $one){
            if($this->sanitize_operators($one[1])){
                $value = $this->sanitize_values($one[2]);
                $this->statement = $this->statement.$one[0].' '.$one[1].' '."'$one[2]'";
                $this->statement = $this->statement.' AND ';
            } else {
                $this->valid = false;   
            }
        }
        $this->statement = substr($this->statement, 0, -4);
        return $this;
    }
